...
def train():
    ...
    train_texts, val_texts, train_st1_labels, val_st1_labels, train_additional_features, val_additional_features = train_test_split(
        df['text'], product_category_labels, additional_features, test_size=0.2, random_state=41
    )
    ...
    model_st1_hazard = ST1Classifier.from_pretrained()
    
    # Initialize Models
    model_st1 = ST1Classifier(
        num_categories=len(le_product_category.classes_),
        additional_feature_size=additional_features.shape[1]
    )

    label_names = le_product_category.classes_
    # Initialize Trainer
